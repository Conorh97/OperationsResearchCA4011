...
def main():
	...
	if args.single_server:
		server_list = [Server(completion_rate=0.5)]
	else:
		server_list = [Server(completion_rate=1.0), Server(completion_rate=2.0), Server(completion_rate=2.0)] 
	...
	for i in range(service_minutes):
		for server in server_list:
			...
			if not server.is_serving and len(q) > 0:
				q_client = q.pop()
				q_client.set_queue_time(i)
				q_client.estimated_time = round(q_client.estimated_time * server.completion_rate)
		...
		if i in intervals:
			...
			for server in server_list:
				if not server.is_serving and not assigned_to_server:
					client = Client(i, round(mu * server.completion_rate), 0)
